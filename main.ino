...
void loop() {
    ...
    if (millis() - lastOptimizationTime > OPTIMIZATION_INTERVAL) {
        ...
        totalEfficiency /= (TEST_DURATION / 100);
        optimizer.update(totalEfficiency);
    }

    // Q-learning update process
    float error = voltageIn[0] - VOLTAGE_SETPOINT;
    int state = discretizeStateAgent3(error, generatorLoad);
    int action = chooseActionAgent3(state);

    executeActionAgent3(action);

    float reward = calculateRewardAgent3(efficiency, voltageIn[0], voltageDrop, powerOutput);
    int nextState = discretizeStateAgent3(error, generatorLoad);
