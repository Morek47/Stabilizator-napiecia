...
float calculateEfficiency(float voltageIn, float currentIn,
    ...
    float inputPower = voltageIn * currentIn; // Moc z jednego czujnika prądowego
...
void loop() {
    ...
    // Logowanie danych
    // logData(); // Funkcja logowania danych (niezdefiniowana w dostarczonym kodzie)

    // Sprawdzenie alarmów
    // checkAlarm(); // Funkcja sprawdzająca alarmy (niezdefiniowana w dostarczonym kodzie)

    // Automatyczna kalibracja
    // autoCalibrate(); // Funkcja automatycznej kalibracji (niezdefiniowana w dostarczonym kodzie)

    // Zarządzanie energią
    // energyManagement(); // Funkcja zarządzania energią (niezdefiniowana w dostarczonym kodzie)
